...

{
  ...
  check("text editor is transparent (no white box)", /\.edit-obj textarea\s*\{[^}]*background:\s*transparent/.test(uiSrc));
  check("editor and preview share one box metric", /applyTextBoxMetrics\((area|preview), obj, displayScale\(\)\)/.test(boardSrc));
  check("no double padding on the text node", !/node\.style\.padding/.test(boardSrc));
  check("text tool disarms after stamping", /partial\?\.type === "text"\) disarmToSelect\(\)/.test(appSrc));
  check("text strip follows a text selection", /singleSelectedObject\(\)\?\.type === "text"\) return "text"/.test(appSrc));
